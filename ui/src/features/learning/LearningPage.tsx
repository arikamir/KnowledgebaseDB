...
export function LearningPage() {
  ...
  return <><h1>Focused learning</h1><LearningSession session={state.session} onComplete={state.completeStep} />{lab && <LabExperience lab={lab} onReport={(reason) => state.reportLab(lab.id, reason)} onReturn={state.load} />}{!state.attempt && !state.completion && <button onClick={() => void state.beginReview()}>Begin review</button>}{state.attempt && !state.completion && <SessionReview attempt={state.attempt} feedback={state.feedback} onAnswer={state.answer} onSubmit={state.submitReview} />}{state.completion && <CompletionPanel result={state.completion} />}<PersistenceStatus snapshot={state.snapshot} />{state.problem && <p role="alert">{state.problem === "REVIEW_RETRY_RATE_LIMITED" ? "Try another review in 15 minutes." : "Your input is preserved. Try again."}</p>}</>;
}
