...
export function RoadmapForm({ input, onChange, onSubmit, persistence }: { input: RoadmapInput; onChange: (value: RoadmapInput) => void; onSubmit: () => Promise<void>; persistence: PersistenceSnapshot }) {
  ...
  return (
    <form onSubmit={submit} noValidate>
      ...
      <label>Current role<input aria-describedby={errors.role ? "role-error" : undefined} aria-invalid={Boolean(errors.role)} required value={input.role} onChange={(event) => onChange({ ...input, role: event.target.value })} /></label>{errors.role && <p id="role-error" role="alert">{errors.role}</p>}
      <label>Experience level<select aria-describedby={errors.experienceLevel ? "level-error" : undefined} aria-invalid={Boolean(errors.experienceLevel)} required value={input.experienceLevel} onChange={(event) => onChange({ ...input, experienceLevel: event.target.value })}><option value="">Select level</option><option value="beginner">Beginner</option><option value="intermediate">Intermediate</option><option value="advanced">Advanced</option></select></label>{errors.experienceLevel && <p id="level-error" role="alert">{errors.experienceLevel}</p>}
      <label>Target role<input aria-describedby={errors.targetRole ? "target-error" : undefined} aria-invalid={Boolean(errors.targetRole)} required value={input.targetRole} onChange={(event) => onChange({ ...input, targetRole: event.target.value })} /></label>{errors.targetRole && <p id="target-error" role="alert">{errors.targetRole}</p>}
      <label>Hours available per week<input aria-describedby={errors.availableTimePerWeek ? "hours-error" : undefined} aria-invalid={Boolean(errors.availableTimePerWeek)} required min={1} max={40} type="number" value={input.availableTimePerWeek || ""} onChange={(event) => onChange({ ...input, availableTimePerWeek: Number(event.target.value) })} /></label>{errors.availableTimePerWeek && <p id="hours-error" role="alert">{errors.availableTimePerWeek}</p>}
