...
- **SC-043**: Core performance verification MUST run two independent scenarios: roadmap creation and skill guidance. Each scenario MUST use 10 concurrent workers and a versioned fixture set containing approved beginner, intermediate, and advanced employee profiles and supported guidance topics. Each worker MUST complete two successful warm-up requests that are excluded from results, followed by 10 measured requests, producing 100 measured attempts per scenario. Core execution MUST be measured with a monotonic clock from core ASGI request entry through completion of response serialization, excluding BFF transport and browser rendering. Using the nearest-rank percentile calculation, at least 95 of 100 valid roadmap attempts MUST complete within 30 seconds and at least 95 of 100 valid guidance attempts MUST complete within 10 seconds. Failed or timed-out valid requests MUST remain in the denominator and count as threshold failures. Evidence MUST record the performance-profile version, fixture-set digest, environment, image digest, individual timings, outcome, and calculated p95.
...
- The first release implements and verifies the explicit accessibility behaviors in FR-013, FR-014, SC-005, and SC-006. Formal certification of complete WCAG 2.2 Level AA conformance is outside this feature.
