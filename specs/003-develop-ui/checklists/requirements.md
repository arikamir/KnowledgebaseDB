...
- GitHub Actions was added as an explicit user-mandated delivery constraint. This is a
...
- [x] CHK007 Are GitHub Actions requirements complete for pull requests, protected refs, component selection, validation, publication, promotion, deployment, evidence, concurrency, outages, and rollback? [Gap, Spec §FR-064, SC-033]
...
- [x] CHK014 Are GitHub Actions protected refs, affected-component rules, required validation gates, deployment target, and evidence retention defined without relying on implicit job configuration? [Clarity, Spec §FR-064, SC-033]
...
- [x] CHK019 Are the implementation-specific Azure and GitHub Actions constraints clearly separated from technology-agnostic user outcomes without weakening traceability between them? [Consistency, Spec §FR-049–FR-052, §FR-064, SC-025–SC-033]
...
- [x] CHK030 Are requirements defined for GitHub Actions cancellation, agent loss, Azure authentication denial, ACR publication without promotion, stale builds, and simultaneous deployment requests? [Coverage, Spec §FR-064, SC-033]
...
- [x] CHK033 Are security requirements explicit for token boundaries, CSRF, session fixation, cookie attributes, log redaction, GitHub Actions ACI managed identities, and least-privilege failure behavior? [Completeness, Spec §FR-040, §FR-053–FR-059, §FR-064]
...
- [x] CHK037 Are external dependencies and their failure contracts documented for Entra, Redis, PostgreSQL, ACR, AKS, gateway, Key Vault, Azure Monitor, lab providers, and GitHub Actions? [Dependency, Gap]
...
- [x] CHK039 Are the authentication and learning contracts consistent with the latest clarifications, data ownership model, GitHub Actions delivery contract, and task boundaries? [Consistency, Contracts §auth-ui, §learning-ui, §github_actions-delivery]
- [x] CHK040 Is the stale readiness note about regenerating the plan and tasks clearly superseded now that GitHub Actions-aware plan and task artifacts exist? [Conflict, Checklist §Notes]
