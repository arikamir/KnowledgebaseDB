...
mkdir -p "$target"
for file in versions.tf variables.tf main.tf github-actions-identities.tf delivery-evidence-storage.tf outputs.tf terraform.tfvars.example .gitignore .terraform.lock.hcl; do
